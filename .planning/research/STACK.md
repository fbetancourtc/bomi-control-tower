...
**Domain:** Real-time logistics dashboard (Python + FastAPI + Google Sheets)
**Researched:** 2026-03-31
**Confidence:** MEDIUM (versions based on training data up to May 2025 + existing Diageo project patterns; unable to verify latest PyPI versions due to tool restrictions)
...
# Dev dependencies
pip install ruff pytest pre-commit pip-audit mypy
```
...
**If real-time updates are critical (sub-5-second freshness):**
...
**If near-real-time is acceptable (30-60 second freshness):**
...
**If you need to support >20 concurrent users:**
...
**If you later need a database (v2+):**
...
The following files from `/Users/fbetncourtc/Documents/finops/proceso-tarificacion-diageo/` should be copied and adapted:
...
For a simple internal tool with 3-5 users, use **cookie-based JWT sessions**:

```
Login flow:
1. GET /login -> Render login form (Jinja2)
2. POST /login -> Verify username/password against hardcoded user list
3. Set HTTP-only cookie with JWT token (expires in 8 hours)
...
This is deliberately simple. Do not add OAuth2 flows, RBAC, or user management UI for v1.
...
**Version confidence note:** I was unable to verify latest PyPI versions due to tool restrictions (WebSearch, WebFetch, and Bash all denied). The version minimums specified (e.g., `>=0.115.0` for FastAPI) are conservative floors that are known-good. Running `pip install --upgrade` at project setup time will pull the actual latest compatible versions.

---
*Stack research for: Real-time logistics dashboard (Python + FastAPI + Google Sheets)*
*Researched: 2026-03-31*
